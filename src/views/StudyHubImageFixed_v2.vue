...
function openImagePicker(mode = "cursor") {
  ...
  // Reset value first, otherwise choosing the same image again may not trigger change.
  input.value = "";
  input.click();
}
...
async function handleImagePick(event) {
  ...
  try {
    for (const file of files) {
      // Important: insert from FileReader immediately.
      // Do not wait for Firebase Storage, because Storage rules / env config can fail and make the editor look broken.
      const localImageUrl = await createLocalImageDataUrl(file);
      const insertedImage = insertImageAtCursor(
        localImageUrl,
        file.name || "Inserted image",
        insertMode,
      );
      syncImageToFirebaseInBackground(file, insertedImage);
      insertedCount += 1;
    }
...
async function handleEditorPaste(event) {
  ...
  try {
    ...
    showToast("Image could not be pasted");
  } finally {
    imageBusy.value = false;
  }
}
...
async function handleEditorDrop(event) {
  ...
  try {
    ...
    showToast("Image could not be added");
  } finally {
    imageBusy.value = false;
  }
}
...
function createLocalImageDataUrl(file) {
  // SVG and GIF should not be rendered through canvas because they may lose animation / fail in some browsers.
  if (/image\/(gif|svg\+xml)/i.test(file.type)) {
    return fileToDataUrl(file);
  }

  // Compress large screenshots/photos before saving them inside the note HTML.
  // This avoids localStorage / Firestore document size problems.
...
async function resolveImageSource(file) {
  // Kept for compatibility with older calls, but image insertion now uses local FileReader first.
  return createLocalImageDataUrl(file);
}

async function uploadImageToFirebase(file) {
  if (!FIREBASE_SYNC_ENABLED || !firebaseStorage) return null;
...
function syncImageToFirebaseInBackground(file, imageElement) {
  // The image has already been inserted locally. Firebase upload is only a background optimisation.
  // If Storage fails, the editor still works and the local image stays visible.
  uploadImageToFirebase(file).then((remoteUrl) => {
    if (!remoteUrl || !imageElement?.isConnected) return;
...
function insertImageAtCursor(src, alt = "Study image", mode = "cursor") {
  ...
  const insertedImage = figure.querySelector("img");
  ...
  try {
    ...
    console.error("Direct image insert failed, appending instead:", error);
    editorRef.value.appendChild(beforeParagraph);
    editorRef.value.appendChild(figure);
    editorRef.value.appendChild(afterParagraph);
    focusParagraph(afterParagraph);
    return insertedImage;
  }
}
